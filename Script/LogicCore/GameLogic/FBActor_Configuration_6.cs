			...
			get
			{
				if (s_default == null)
				{
					s_default = new Configuration() {
						...
						bodyHeight = (Fix64)1.6f,
						//球员身高

						catchingRadius = (Fix64)0.0f,
						...
						maxCatchingBallHelperHeight = (Fix64)2.0f,
						//自动追球辅助时，足球最大允许高度
						...
						//普通移动
						normalSpeed = new Fix64[] { (Fix64)5.5f, (Fix64)5.5f, (Fix64)5.5f, (Fix64)5.5f, (Fix64)5.5f },
						m1_maxSpeed = (Fix64)6,
						m1_moveForce = (Fix64)18,
						m1_stopDampingAcceleration = (Fix64)24,
						m1_angularSpeed = (Fix64)100,

						//防御姿态移动
						dm1_angleA = (Fix64)(30.0f / 180 * 3.14f),
						dm1_angleB = (Fix64)(120.0f / 180 * 3.14f),
						dm1_maxSpeed = new Fix64[] { (Fix64)6, (Fix64)6, (Fix64)6, (Fix64)6 },

						//带球移动
						normalSpeed_ball = (Fix64)5.5f,
						m1_maxSpeed_ball = (Fix64)6,
						m1_moveForce_ball = (Fix64)18,
						m1_stopDampingAcceleration_ball = (Fix64)24,
						m1_angluarSpeed_ball = (Fix64)100,
						...
						//转身
						m2_minAngleCos = (Fix64)(-1.1f), //180°+
						m2_minSpeedAndWaitTime = new Fix64[] { (Fix64)0.1f, (Fix64)0.1f, (Fix64)(-0.1f), (Fix64)0.1f },
						m2_minSpeedAndWaitTime_ball = new Fix64[] { (Fix64)0.1f, (Fix64)0.1f, (Fix64)(-0.1f), (Fix64)0.1f },

						m2_movingTime = (Fix64)0.2f,

						//铲球
						st_initialSpeed = (Fix64)40,
						st_dampingToZeroTime = (Fix64)0.3f,
						st_waitingTime = (Fix64)0.2f,
						st_maxSlidingTargetDistance = (Fix64)3.5f,
						st_maxSlidingTargetAngle = (Fix64)(180.0f / 180 * 3.14f), 

						//被铲
						bst1_initialSpeed = (Fix64)35.0f,
						bst1_dampingToZeroTime = (Fix64)0.6f,
						//bst1_standWaitingTime = (Fix64)1.25f,
						bst1_waitingTime = (Fix64)1.6f,
						st_beAttackedFaceBackoff = 1,
						...
						//传球
						pb_beforePassingTime = new Fix64[] { (Fix64)0.23f, (Fix64)0.23f },
						pb_afterPassingTime = new Fix64[] { (Fix64)0.57f, (Fix64)0.57f },

						//射门
						sb_angularSpeed = (Fix64)6,
						sb_beforeShootingTime = new Fix64[] { (Fix64)0.13f, (Fix64)0.13f, (Fix64)0.16f, (Fix64)2.4f },
						...
						scb_maxRadius = (Fix64)1.84f,
						scb_maxCathingTime = (Fix64)0.3f,
						scb_catchingAniTime = new Fix64[]{ (Fix64)0.2f, (Fix64)0.13f, (Fix64)0.17f},
						scb_catchingHeightLimit = new Fix64[]{ (Fix64)1.0f, (Fix64)2.5f, (Fix64)4.0f},
						scb_catchingOffset =  new Fix64[] { (Fix64)1.84f, (Fix64)0.79f, (Fix64)1.13f},
						scb_catchingOffsetH = new Fix64[] { (Fix64)0.31f, (Fix64)1.82f, (Fix64)3.13f },
						scb_lockTimeAfterCatching = new Fix64[] { (Fix64)1.0f, (Fix64)0.87f, (Fix64)0.63f },
						...
						tcb_glideTimeAfterCatching = (Fix64)0.3f,
						tcb_lockTimeAfterCatching = (Fix64)0.3f,


						ballAttachPositions = new FixVector3[]
						{
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.84 },
							//站立接短传
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.84 },
							//跑步接短传
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.84 },
							//站立接长传
							new FixVector3 { x = (Fix64)0, y = (Fix64)3.13, z = (Fix64)1.13 },
							//跑步接长传
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.84 },
						},
						...
						{
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.6 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.6 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.6 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.6 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.6 },
						...
						lastBallSamplePositions = new FixVector3[]
						{
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.84 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.84 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)1.82, z = (Fix64)0.79 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)3.13, z = (Fix64)1.13 },
						},

						flyCatchingBallFreezingTime = (Fix64)0.1f,
						//守门员扑球范围0123456对应ABCDEFG参数（详见文档），长度为绝对范围
						dkcb_edgeLimit = new Fix64[] { (Fix64)1.2f, (Fix64)1.0f, (Fix64)3.2f, (Fix64)5.5f, (Fix64)2.0f, (Fix64)5.5f, (Fix64)5.0f },                        
						//对应 0-a区  1-b区 2-c区 3-d、g区 4-f、e区
						//守门员在每个区接球时对应的位置偏移 x 对应水平偏移， y对应垂直偏移
						dkcb_cathingOffset = new FixVector2[] 
						{
							...
							new FixVector2{ x = (Fix64)0, y = (Fix64)0 },
							new FixVector2{ x = (Fix64)0, y = (Fix64)3.27f },
							new FixVector2{ x = (Fix64)0.956f, y = (Fix64)1.99f },
							new FixVector2{ x = (Fix64)0.836f, y = (Fix64)0.31f },
						},
						//守门员接球时动画所需要的时间
						dkcb_animationCathingTime = new Fix64[] { (Fix64)0.2f, (Fix64)0.13f, (Fix64)0.2f, (Fix64)0.2f, (Fix64)0.2f },
						//守门员空中接球落地后滑行等待时间
						dkcb_afterFallingGlideTime = new Fix64[] { (Fix64)0.0f, (Fix64)0.0f, (Fix64)0.0f, (Fix64)0.4f, (Fix64)0.4f },
						//守门员接球后等待时间对应 0-a区  1-b区 2-c区 3-d、g区 4-f、e区
						dkcb_afterCathingWaitingTime = new Fix64[] { (Fix64)1.0f, (Fix64)0.87f, (Fix64)0.66f, (Fix64)0.6f, (Fix64)0.6f },
						//守门员在每个区的接球移动速度 ab区配置为0
