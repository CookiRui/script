            ...
            get
            {
                if (s_default == null)
                {
                    s_default = new Configuration() {
						...
						bodyHeight = (Fix64)2.6f,
						//球员身高

                        catchingRadius = (Fix64)1.1f,
						...
						//普通移动
                        normalSpeed = new Fix64[] { (Fix64)5.9f, (Fix64)0, (Fix64)0, (Fix64)0, (Fix64)0 },
						//美术提供的制作数据
                        m1_maxSpeed = (Fix64)7.2,
						//角色移动速度
                        m1_moveForce = (Fix64)21.6,
						//角色启动速度
						m1_stopDampingAcceleration = (Fix64)28.8,
						//角色制动速度
                        ...
                        //带球移动
						normalSpeed_ball = (Fix64)5.9f,
                        m1_maxSpeed_ball = (Fix64)6.8,
                        m1_moveForce_ball = (Fix64)20.4,
						m1_stopDampingAcceleration_ball = (Fix64)27.2,
                        ...
                        //急停转身的最大角度，一般不会改变
                        m2_minSpeedAndWaitTime = new Fix64[] { (Fix64)0.1f, (Fix64)0.3f, (Fix64)(-0.1f), (Fix64)0.3f },
						//无球，速度大于某时的动画时间、速度小于某时的动画时间
                        m2_minSpeedAndWaitTime_ball = new Fix64[] { (Fix64)0.1f, (Fix64)0.3f, (Fix64)(-0.1f), (Fix64)0.3f },
						...
						//被铲
                        bst1_initialSpeed = (Fix64)12.0f,
						//攻击目标受击后初速度
                        bst1_dampingToZeroTime = (Fix64)0.95f,
						//攻击目标受击后移动到停止的时间
                        ...
                        //短传、长传出球时间
                        pb_afterPassingTime = new Fix64[] { (Fix64)0.2f, (Fix64)0.5f },
						 //短传、长传出球后时间

						//射门
                        sb_angularSpeed = (Fix64)6,
						//射门转身角速度，统一数据
                        sb_beforeShootingTime = new Fix64[] { (Fix64)0.23f, (Fix64)0.2f, (Fix64)0.26f, (Fix64)2.73f },
                        sb_afterShootingTime = new Fix64[] { (Fix64)0.9f, (Fix64)0.5f, (Fix64)0.66f, (Fix64)0.7f },
						...
						//辅助接球时角色的最大速度
                        scb_maxRadius = (Fix64)1.01f,
						//用脚接球的最大半径
                        scb_maxCathingTime = (Fix64)0.3f,
						//检测到在本时间内经过角色的符合标准的足球，进行接球辅助
						scb_catchingAniTime = new Fix64[]{ (Fix64)0.2f, (Fix64)0.16f, (Fix64)0.1f},
						//角色停球需要的时间：用脚、用胸、用手
                        scb_catchingHeightLimit = new Fix64[]{ (Fix64)1.0f, (Fix64)3.0f, (Fix64)0.0f},
						//导致上述接球方式的足球高度范围
						scb_catchingOffset =  new Fix64[] { (Fix64)1.01f, (Fix64)0.71f, (Fix64)0.0f},
						scb_catchingOffsetH = new Fix64[] { (Fix64)0.31f, (Fix64)2.26f, (Fix64)0.0f },
						//最佳停球点，半径和高度
						scb_lockTimeAfterCatching = new Fix64[] { (Fix64)0.3f, (Fix64)0.33f, (Fix64)0.0f },
						//停球动作后摇，从停到球开始计算
                        ...
                        {
                            new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.98 },
                            //默认
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.01 },
							//站立接短传
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.1 },
							//跑步接短传
							new FixVector3 { x = (Fix64)0, y = (Fix64)2.26, z = (Fix64)0.71 },
							//站立接长传
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)1.1 },
							//跑步接长传
                        },
                        ...
                        ballDetachPositions = new FixVector3[]
                        {
							...
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.98 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)0.31, z = (Fix64)0.98 },
							new FixVector3 { x = (Fix64)0, y = (Fix64)11.59, z = (Fix64)1.89 },
                        },
