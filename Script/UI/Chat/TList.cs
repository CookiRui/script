...
{
	/// <summary>
	/// If Direct access to the buffer,Note that you should not use its 'Length' parameter.
	/// </summary>

    protected T[] buffer;
    private const int DEFAULT_CAPACITY = 32;
    /// <summary>
    /// Direct access to the buffer's size by Count.
    /// </summary>
	...
	public T this[int i]
	{
        //get { if (i >= 0 && i < this.size) return buffer[i]; else return default(T); }
        //set { if (i >= 0 && i < this.size) buffer[i] = value; }
    ...
    void AllocateMore()
    {
        T[] newList = (buffer != null) ? new T[Mathf.Max(buffer.Length << 1, DEFAULT_CAPACITY)] : new T[DEFAULT_CAPACITY];
	...
	/// <summary>
	/// Clear the array by resetting its size to zero. The memory is actually released.
	/// </summary>

    public void Clear() { size = 0; buffer = null; }
