...
[AddComponentMenu("UI/Effects/TextSpacingSetting",101)]
public class TextSpacingSetting : BaseMeshEffect
{
    ...
    public override void ModifyMesh(VertexHelper vh)
    {
        ...
        string[] lineTexts = new string[text.cachedTextGenerator.lineCount];
        UILineInfo[] uiinfor = text.cachedTextGenerator.GetLinesArray();
        int start = 0; int end = 0; 
        for (int i = 0; i < uiinfor.Length; i++)
        {
            start = uiinfor[i].startCharIdx;
            if (i == uiinfor.Length - 1)
                end = text.text.Length;
            else
                end = uiinfor[i + 1].startCharIdx;
            lineTexts[i] = text.text.Substring(start, end - start);
        }
